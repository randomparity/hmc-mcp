...
The site is latent, not live. `_lpar_job` is private, all five callers are in the same module,
and each passes a literal — `Migrate`, `MigrateValidate`, `MigrateAbort`, `MigrateRecover`,
`RemoteRestart` (`client_lpm.py:50`, `:71`, `:78`, `:86`, `:110`). What a rule buys is that the
sixth call site cannot introduce a caller-supplied value silently.

Two facts decide the shape. The namespace here is **closed**, which the uom type namespace is
not: ADR 0143 accepts that no published list of HMC type names exists and so had to write a
character grammar, while the operations this client submits are exactly the five its own code
names. And the drift test that would have caught this site cannot see it — `_uom_path_sites()`
in `tests/unit/test_request_path_safety.py` parses `hmc_mcp.client.core` alone — so a test-time
observation is not available here. Widening that walk is issue #828's other half, excluded here.
...
**Membership, not a grammar.** ADR 0147 forecast a grammar here but deferred the question rather
than deciding it. `[A-Za-z][A-Za-z0-9]*` would admit `PowerOff` and `DeleteVirtualAdapters` —
real HMC job operations this client does not submit — where membership admits neither. A closed
set is available precisely because the argument is private and every caller is in the file.
...
already follow. The set lives in `client_lpm.py` rather than `client_contracts.py`: it governs
one function in one module and has no second caller.
...
- An `operation` outside the five is refused with a `ValueError` before `submit_job` is called,
  ...
  `tests/lpar/test_lpm.py:101-152` routes each literal through `respx` and reads
  `route.calls.last`, so a sixth call site whose literal is missing reddens its own test.
- **The set has to be edited to add an operation** — the deliberate cost of closing the
  namespace: one line beside the new call site, at the moment someone decides it is safe.
...
- ADR 0147's follow-up candidate for `operation` is closed; the seven other ungoverned segment
  arguments it enumerated stay open and unowned.

## Considered & rejected

- **A character grammar, as ADR 0147 forecast.** verified: all five literals match `_UOM_TYPE`'s
  `[A-Za-z][A-Za-z0-9]*` (`client_contracts.py:39`), so a grammar would accept every live caller.
  judgment: it would equally accept every other HMC job operation and any invented name in that
  class — weaker than the closed namespace supports.
...
- **Widen `_uom_path_sites()` to reach `client_lpm.py`.** verified: ADR 0147 measured that the
  same walk over `src/hmc_mcp/client/*.py` surfaces eight unclassified names. judgment: excluded
  by the operator; it needs its own PR to fix or classify what it finds.
