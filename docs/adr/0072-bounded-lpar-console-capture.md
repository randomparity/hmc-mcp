...
> EOF and reads to stream end, bounded by the same 20 s, and issues `rmvterm` only when that
> wait times out, fails, or finds the connection closed. A lost-hold report during that wait
...
> held found it held, acquired after the probe's `mkvterm` exited, and kept its session. Decision
