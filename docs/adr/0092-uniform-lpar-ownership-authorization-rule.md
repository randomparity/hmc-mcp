...
description, and `authorize_lpar_mutation` (`src/hmc_mcp/operations/lpar.py:502`)
...
`delete_lpar` (`operations/lpar.py:680`, guard at `:807`), `rename_lpar` (`:904`,
...
| Operation | Location | Status | Tracking |
|---|---|---|---|
| `set_lpar_boot_order` | `operations/lpar.py:953` | guarded (`:997`) | — |
...
| `set_lpar_processors` | `operations/lpar.py:838` | guarded (`:825`, via `_apply_dlpar_document:817` → `_resolve_and_authorize_lpar:725`) | — |
| `set_lpar_memory` | `operations/lpar.py:874` | guarded (`:825`, via `_apply_dlpar_document`) | — |
...
| `attach_disk_to_lpar` | `operations/provision.py:343` | **unguarded** | #372 |
...
|---|---|---|---|
| `hmc_set_lpar_msp` | `server_tools/lpar_config.py:351` | **unguarded** | #441 |
| `hmc_set_lpar_proc_compat` | `server_tools/lpar_config.py:400` | **unguarded** | #441 |
| `hmc_modify_lpar` | `server_tools/lpars.py:197` | **partially guarded** — the `assignments` leg delegates to guarded operations, the `resources` leg calls `modify_logical_partition` with no ownership check | #442 |
...
| Operation | Location | Status | Tracking |
|---|---|---|---|
| `power_lpar` | `operations/lpar.py:403` | guarded when opted in (`:442`, via `_resolve_and_authorize_lpar`); §4 | #371 |
...
| Operation | Reason |
|---|---|
| `create_and_stamp_lpar` (`operations/lpar.py:259`) | Creates the partition. No prior owner exists to authorize against; it stamps the token instead (ADR 0011). |
