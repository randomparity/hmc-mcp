...
- **`hmc_run_command` is omitted.** Epic requirement 6 keeps the arbitrary-command escape hatch a
  distinct maximum-risk capability that `--enable-arbitrary-command` alone does not confer. A
  generator that granted it would make the flag sufficient again. The generated file carries a
  comment saying how to add it.
...
`connections = ("<default>",)` and pass it to `create_mcp`. The smoke path therefore proves on
every CI leg that the document the generator emits actually compiles and composes.
...
- **Legacy-equivalent is not legacy.** Under the generated policy every connection-bearing tool
  ...
  and every decision is recorded. The authorization outcome is the same as before — permitted —
  but the cost, the log volume, and the failure modes are not. An operator reading
...
- **The generated policy pins the tool surface at generation time.** After an upgrade that adds a
  ...
  deliberate; the generated file's header says to regenerate after upgrading, and
  `hmc_effective_permissions` is how an operator sees the difference.
...
- **Importing `hmc_mcp.server` no longer composes an application.** `mcp = create_mcp()` ran at
  import and registered 130 tools; removing it makes the import cheap and makes any code that
