...
Accepted on 2026-09-21, after review findings were fixed on a branch merged up to
`main@4db1545a`: 35/35 unit tests in `tests/scripts/test_pcie.py` pass, `just verify` exits 0,
and `uv run --no-sync prek run --all-files` passes on every hook.

**No live run has exercised this version of the arm.** An earlier revision was run against
hardware on 2026-09-03, but that revision could not import against current `main`, read its
configuration from the ambient environment, matched `io_slots` by substring, and treated a
lost create response as "nothing was created" — so its PASS/SKIP/FAIL matrix is not evidence
...
`operations/virtualization/pcie.py:require_admitted_environment` enforces for the SR-IOV path.
...
- **Build the arm only on `hmc_assign_dedicated_pcie_slot` / `hmc_unassign_dedicated_pcie_slot`.**
  verified: `operations/virtualization/pcie.py:_authorize_pcie_profile_request` raises
  ...
  non-empty `assignments.dedicated`, both re-checked at `main@4db1545a`. Every run would be
  SKIP and no `io_slots` evidence would ever be produced.
...
- **Reuse the configured long-lived partition, as the SR-IOV arm does.** verified: issue
  #217 requires "a run-unique owner-stamped LPAR" and deletion "only after
  UUID/run-marker/ownership comparisons prove this run still owns it"; that partition
  predates the run and carries no run marker, so no comparison could establish ownership.
- **Name the arm's inputs `HMC_*` and read them from the environment.** verified: ADR 0115
  owns live-test configuration and rejects both — `HMC_*` is `HMCConfig`'s namespace, and
  `live_test_runner.py:_load_dotenv` skips a key already present in `os.environ`, so an
  ambient export would silently outrank the reviewed `.env` and choose the managed system
  this arm creates and deletes partitions on. The four inputs are `LIVE_TEST_DEDICATED_PCIE_*`
  in `LiveTestConfig._OPTIONAL_CONFIG_FIELDS`, read from that file and validated before the
  client is built. They are optional rather than required because they configure one opt-in
  arm: absent, the arm SKIPs; it never falls back to `LiveTestConfig.system_name`.
