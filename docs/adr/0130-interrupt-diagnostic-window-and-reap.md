...
`KeyboardInterrupt` report on a loaded host. ADR 0129 named this as a residual it could not
take, the constant being in a file that change could not touch.
...
method, whole process tree pinned to one core with `taskset` alongside N busy-loop spinners;
x86_64, 48 cores, CPython 3.11.15; 3 repeats per row:
...
ADR 0129 lost it at 80 on its host 1 and at 40 on its host 2: three hosts, three boundaries.

The second wait guards a different quantity. Against a child that ignores the signals,
`SIGTERM`-to-reaped took 3.6 ms, 121 ms, and 224 ms at 0, 40, and 80 spinners, and
`SIGKILL`-to-reaped 2.9 ms, 84 ms, and 345 ms. At the load where the diagnostic needs 16.9 s,
the reap needs 0.22 s.
...
- `INTERRUPT_GRACE_SECONDS = 60` — the diagnostic window, before `SIGTERM`. Sized from a bound
  already in the system rather than from a stopwatch: ADR 0129 set the real-interrupt test's
  readiness ceiling to 60 s, and exit tracks readiness at 0.95x, so a host slow enough to lose
  the diagnostic inside a 60-second window has already failed that ceiling and been reported as
  a hang. The two bounds are now consistent instead of independently chosen.
...
- The timeout arm escalates directly, skipping the window. Nothing has asked that child to stop,
  so waiting cannot help it write anything; without this split, raising the shared constant
  would add 57 s to a path this change has no business slowing.
- The test's collection budget becomes `INTERRUPT_GRACE_SECONDS + TERMINATE_GRACE_SECONDS +
  _INTERRUPT_COLLECTION_SLACK_SECONDS`, keeping ADR 0129's coupling to the module under test.

## Consequences

- ADR 0129's residual is closed. Its decision stands unchanged; the Context finding that
  collection is host-independent is corrected by the table above.
- The real-interrupt test's collection budget rises from 16 s to 73 s and its overall worst case
  from 76 s to 133 s, against the `ci` job's 20-minute leg. Both are paid only by a child that
  ignores `SIGINT`, which is the defect the test exists to report.
- A developer whose pytest is genuinely wedged waits 60 s rather than 6 s unless they press
  `Ctrl-C` again. Two rapid `Ctrl-C`s now truncate the diagnostic deliberately, where before
  they discarded it, misreported the status, and orphaned the child.
- ADR 0129's coupling widens: `TERMINATE_GRACE_SECONDS` now moves the test budget too.
- Every number here comes from one amd64 host on CPython 3.11.15, where ADR 0129 measured two
  ...
  unmeasured, exactly as they were for ADR 0129. Both constants sit far above any plausible leg
...
- **Raise `INTERRUPT_GRACE_SECONDS` and change nothing else.** verified: it is spent on both
  ...
  separating the constants is what makes the larger number answer to a bound in the system
  rather than to a stopwatch, and stops the smaller one paying for it.
- **Escalate on absence of output rather than wall clock** (issue #728's candidate 2).
  ...
  threshold would have to exceed 16.9 s here, and roughly 50 s on ADR 0129's slower host, purely
  to avoid killing the burst it exists to preserve — the same constant racing the same host
  speed, wrapped in more machinery.
- **Derive the window from a host measurement taken at run time** (issue #728's candidate 1).
  ...
  how long pytest ran before the interrupt, which is set by the suite's length and the user's
  patience rather than by the host. judgment: the derivation has no input, and a synthetic
  calibration loop is machinery this path does not need.
- **Leave both waits on one constant and raise it.** verified: reap latency stayed under 0.35 s
  at every load measured, against 16.9 s for the diagnostic. judgment: one constant spanning
  quantities two orders of magnitude apart is what forced the test budget to `2 * grace` and
  what makes a raise cost more than it buys.
