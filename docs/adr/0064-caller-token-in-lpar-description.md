...
- **Fold the caller token into the ownership stamp's brackets.**
  ...
  (`hmc_mcp/operations/lpar/ownership.py:_OWNERSHIP_TOKEN`) anchors `]` immediately after
  ...
  (operations/lpar/ownership.py:152-167) then fails closed — raising `PermissionError`
