...
caller passed — about 5,461 characters of astral input, about 65,500 of ASCII. Verified:
...
limit this repository can name. Half of that buffer — 4,096 characters — is the ceiling for a
single caller-supplied value, so that no one value can spend more of the line than everything else
on it combined. 3 KiB is the round figure under that ceiling, and divided by the twelve characters
an input character can become, it gives 256.
...
`property_value` literal in `src/` or `tests/` is 31. Above: the first length httpx itself refuses
is ~5,461 in the worst case. 256 is seven times the longest known legitimate value and a
...
- **`console_uuid` is bounded on seven of the nine `UsersMixin` entry points that take it.**
  `get_remote_access` (`client_users.py:111`) and `configure_remote_access` (`:132`) build
...
- No wire-format change for any value this repository passes, and no existing test input is newly
  refused: the longest relevant literal in `src/` or `tests/` is 31 characters.
...
- **Bound at 5,461, the first length httpx actually refuses in the worst case.** verified: it is
  the true ceiling for astral input. judgment: it makes the local refusal a restatement of the
  library's, keeps a 64 KiB URL buildable from ASCII input, and leaves nothing for a stricter
  server.
