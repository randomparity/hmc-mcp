...
3. Add operation/source/target forwarding through `client_lpm.py` and `operations_lpm.py`, retaining
   existing normalized result types.
4. Expose the explicit contract through `server_lpm.py` and `cli_lpars.py`; update their tests.
