...
| A 204 returns no names rather than raising | `focused-test` | ~~`::test_list_search_parameters_204_returns_no_names`~~ — an implementation raising on any non-200 gives `HMCError`. Renamed `test_list_search_parameters_204_returns_an_unknown_answer` (`tests/unit/test_client.py:3514`); ADR 0144 changed the 204 return to `(None, version)`. |
| ~~A 200 yielding no name raises, rather than reporting "defines nothing"~~. Raises only when the container is absent; ADR 0144 makes the container-present, no-usable-name case report "defines nothing" instead | `focused-test` | ~~`::test_list_search_parameters_200_without_a_name_raises`, parametrized over an `HttpErrorResponse` feed and an empty collection — `Failed: DID NOT RAISE`~~. Split under ADR 0144 into `test_list_search_parameters_200_without_the_container_raises` (`tests/unit/test_client.py:3530`, still raises on the `HttpErrorResponse` shape), `test_list_search_parameters_empty_set_returns_no_names` (`:3551`, returns `([], "V1_0")`), `test_list_search_parameters_all_empty_names_return_an_unknown_answer` (`:3573`, returns `(None, "V1_0")`), and `test_list_search_parameters_unnamed_items_return_an_unknown_answer` (`:3601`, returns `(None, "V1_0")`). |
