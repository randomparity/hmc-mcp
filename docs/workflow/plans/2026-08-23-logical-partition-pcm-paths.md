...
## Task 1: Resolve an owned PCM target
...
consume it and pass `system_uuid` keyword-only to client methods.
...
2. Run `uv run pytest -q tests/unit/test_pcm.py`; expect the new tests to fail.
3. Implement the dataclass and resolution contract, threading the optional
   selector through metric operations.
...
## Task 2: Build documented client paths and reject invalid endpoints

Files: `src/hmc_mcp/client_pcm.py`, `tests/unit/test_pcm.py`.

Interfaces: `get_processed_metric_links` and `get_aggregated_metric_links` gain
keyword-only `system_uuid: str | None = None`; `_metrics_links` consumes it.
`get_ltm_metric_links` validates that its category is `ManagedSystem`.
...
2. Run the focused test; expect failures for flat paths and missing rejection.
3. Add one path builder with explicit category/owner validation and use it from
   the three metric methods.
4. Run the focused test; expect success, then commit.

Acceptance: exact documented URLs are requested and invalid combinations issue
no request.
