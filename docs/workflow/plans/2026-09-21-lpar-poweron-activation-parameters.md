...
Expected implementation size: 170–260 changed lines (M) — derived from the file map
below: three source functions and two small vocabulary blocks, four test modules, and one
...
- `docs/tools/` is generated. When `just tool-docs-check` fails, run `just tool-docs`
  and commit the result; never hand-edit a file under `docs/tools/`.
...
and every existing call site keeps its present behaviour.
...
- **Contract: a no-argument call emits today's document unchanged.**
  ...
  which pins the literal document copied from `git show main:src/hmc_mcp/jobs/requests.py`.
  Red: the pinned string differs once emission is reordered or a parameter leaks in.
  Green: `uv run --no-sync pytest tests/lpar/test_power.py -k default_document -q`
  reports `1 passed`.
- **Contract: optional parameters are emitted only when supplied.**
  ...
  Green: `uv run --no-sync pytest tests/lpar/test_power.py -k optional_parameters -q`
  reports `1 passed`.
- **Contract: every documented boot mode is accepted.**
  ...
  Green: `uv run --no-sync pytest tests/lpar/test_power.py -k every_boot_mode -q`
  reports `5 passed`.
- **Contract: a non-member is refused before XML is built.**
  Mode: `focused-test`. Test:
  `tests/lpar/test_power.py::test_power_on_lpar_job_rejects_unknown_vocabulary`.
  This test is required rather than inherited: `_is_closed_vocabulary` in
  `tests/unit/test_xml_escaping.py:146` is `get_origin(...) is Literal`, which
  `PowerOnOperationType | None` fails, so the generated vocabulary cases cover
  `bootmode` and not `operation_type`. Red: `TypeError` before the change.
  Green: `uv run --no-sync pytest tests/lpar/test_power.py -k unknown_vocabulary -q`
  reports `2 passed`.
- **Contract: `profile_uuid` is an escaped injection surface.**
  Mode: `focused-test`. Test: generated into `STRING_CASES` by
  `tests/unit/test_xml_escaping.py:210-232`; no new test is written.
  Red: not applicable — the case does not exist before the parameter does.
  Green: `uv run --no-sync pytest tests/unit/test_xml_escaping.py -k power_on_lpar_job -q`
  lists `requests.power_on_lpar_job.profile_uuid.str` and
  `requests.power_on_lpar_job.bootmode` among the ids and reports all passed.
...
1. Before editing any source, capture the document to pin:
   ...
   `hmc_mcp.jobs`.
2. Run `uv run --no-sync pytest tests/lpar/test_power.py -q`. Expect collection errors
...
- **Contract: `power_lpar` forwards all three to the builder on the PowerOn arm.**
  ...
  Green: `uv run --no-sync pytest tests/lpar/test_power.py -k forwards_activation -q`
  reports `1 passed`.
- **Contract: the PowerOff arm is unaffected.**
  ...
  Green: same module, `-k power_off_document`, reports `1 passed`.
- **Contract: `power_on_lpar` passes them through.**
  ...
  Green: same module, `-k passes_activation`, reports `1 passed`.
...
2. Run `uv run --no-sync pytest tests/lpar/test_power.py -q` and observe the three
   `TypeError` failures.
...
- **Contract: the tool forwards all three and names none of them `profile`.**
  ...
  Green: `uv run --no-sync pytest tests/app/test_server_tools.py -k tool_forwards_activation -q`
  reports `1 passed`.
- **Contract: the CLI flags reach the job document.**
  ...
  Green: `uv run --no-sync pytest tests/app/test_cli_commands.py -k activation_flags -q`
  reports `2 passed`.
- **Contract: the generated tool reference matches the registry.**
  ...
  `docs/tools/lpar.md` when the summary line changes and the tree is stale.
  Green: `just tool-docs-check` prints no diff and exits 0.
- **Contract: `hmc_power_on_lpar`'s docstring distinguishes the two profiles.**
  Mode: `task-test-not-applicable`. `scripts/gen_tool_reference.py:154-169` renders only
  the docstring's first line into `docs/tools/`, so no executable consumer reads the
  `Args:` text; a test asserting the wording would assert its own phrasing and would have
  to be edited every time the sentence is improved.

### Steps

1. Write both tests. Define `PROFILE_UUID = "00000000-0000-0000-0000-0000000000aa"` in each
   module beside the existing UUID constants.
2. Run both `-q` commands and observe the two red failures named above.
...
6. Add three parameters to `_power_lpar` with the same names and defaults — it has no
   keyword-only marker today, so append them after `ownership_override` rather than
   introducing one — and forward them into its `power_lpar(...)` call.
   `lpars_power_off` calls `_power_lpar` without them, so the PowerOff path keeps
   today's behaviour through Task 2's PowerOn-arm-only forwarding.
7. Run `just tool-docs`, then `git --no-pager diff --stat docs/tools/`. Commit whatever it
   regenerates without editing it.
8. Run the green commands, then `just lint`, `just typecheck`, `just test`, and
...
9. Commit: `feat(lpar): expose activation parameters on the power-on tool and CLI`.

**Acceptance.** `hmc_power_on_lpar` and `hmc-mcp lpars power-on` each accept the three
parameters; `--boot-mode bogus` exits 2; `docs/tools/` is regenerated; no new parameter is
named `profile` and the docstring says which profile is which.
