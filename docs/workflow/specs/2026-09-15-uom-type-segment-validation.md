...
`src/hmc_mcp/client/core.py` interpolate a caller-supplied `resource_type`,
`parent_type`, or `child_type` raw; the other two interpolate a job id. The same
caller value also reaches the `Accept` media-type parameter via `_uom_headers`.
A type carrying `?` or `#` retargets the request within `/rest/api/uom/`, and
nothing records whether raw interpolation is a decision or an accident. ADR 0143
holds the full evidence and the decision; this spec implements it.

## Scope

ADR 0143 selects boundary validation over percent-encoding. This spec
implements it.

**In scope**

- `_reject_unknown_uom_type(argument, value)` in `src/hmc_mcp/client/core.py`:
  refuses a value not matching `^[A-Za-z][A-Za-z0-9]*$` with `ValueError`
...
- A call on every caller-supplied type argument, in `list_uom`, `get_uom`,
  ...
  covering the `Accept` destination for every caller including `get_uom_path`.
...
- `tests/unit/test_request_path_safety.py`: the grammar, both destinations, the
  refusal-before-transport property, and an AST drift test.
- `tests/unit/test_client.py`: update the three tests (five parametrized cases)
...
- `get_quick_property`'s `property_name` segment. Not a type segment; recorded
  in ADR 0143's consequences as an uncovered adjacent case.
...
- A real HMC type name outside `^[A-Za-z][A-Za-z0-9]*$` would be refused
...
- A malformed type reaches `_uom_headers` only after a path site already
  refused it, so the header check is unreachable for those ten methods.
  Accepted: it is reachable for `get_uom_path` and it makes the invariant a
  property of the function rather than of its caller list.
...
the path sites and inside `_uom_headers` for the header. Fails with
`ValueError` naming the argument and the offending character — no path, no
host, no full caller string, matching `_reject_dot_segments`' leak rule. On the
header boundary it replaces the current control, which is `h11`'s
`normalize_and_validate` refusing a CRLF at send time, in a transitive
dependency this repository neither owns nor tests.
...
- Every caller-supplied type argument named in Scope is refused before a request
  is built when it falls outside `^[A-Za-z][A-Za-z0-9]*$`.
...
- No site drifts, by AST walk of `core.py` — `focused-test`, same file.
