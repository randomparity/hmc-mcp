...
`all_properties` selects `/quick/all` over `/quick`; both parent arguments together select the child
anchor and neither selects the root anchor. The tuple's **first element is the plain list of names**
the issue's outcome asks for; the second is the response's `X-HMC-Schema-Version`, the pairing ADR
0139 established for discovery reads and named this read as inheriting. It reuses
`_request_with_uuid_path_arguments`, which owns UUID validation and reaches `_reject_dot_segments`,
and adds a strict JSON-array-of-strings parse instead of routing the body through `_parse_feed`
(ADR 0140).
...
- **The live response shape of both anchors is unverified at authoring time.** The reference
  documents them as returning defined names; the differently capitalized `/quick/All` is
  live-evidenced to return per-instance value objects (ADR 0138). Accepted because the strict parse
  makes that branch loud — a non-string element raises `HMCError` naming the observed shape, so no
  caller receives objects labelled as names. Confirmation is owed; see *Covered elsewhere*.
...
- A firmware level offering neither anchor, insisting on a typed `Accept`, or not supporting
  `all_properties=True` on the child anchor — a URL the reference does not document, which ADR 0140
  records constructing anyway — answers a non-200 status. Accepted: it surfaces as `HMCError`
  carrying that status and the HMC's own message, the established contract for every read here.
  `/operations` behaved this way at V1_20_0 (ADR 0139) and nothing here remedies a server-side gap.
...
**Boundary inventory.** No new trust boundary. One widened: the HMC REST paths this client
constructs from caller-supplied strings now include the `/quick` and `/quick/all` anchors. Inbound
is the response body and one header; outbound is the request path.

**Actor model.** The untrusted party is the HMC's response, not the caller — the failure model's
first entry names the callers, all in-process, over a session they already authenticated. Trust is
placed in the caller and withheld from the body.

**Control per boundary.** None is new; all are reached by using the existing helper.
- Outbound path — `_reject_dot_segments` inside `_request` refuses raw and percent-encoded
  dot-segments; `_request_with_uuid_path_arguments` refuses a non-UUID `parent_uuid` pre-transport.
- Inbound body — decoded by `json`, then type-checked element by element before any value is
  returned; nothing in it reaches a filesystem path, a command, or a later request here.
- Inbound header — read as an opaque string and returned unchanged, never parsed or used in a path.
- Failure disclosure — `HMCError` carries the status and the body truncated to
  `MAX_ERROR_BODY_BYTES`, as every other read in this module does.

**Explicitly out of scope.** Authorization of who may call this read (#792, not reachable in this
deployment); validating a returned name before a later `get_quick_property` call (epic #785, nothing
calls it here); confidentiality of the name list, firmware metadata identical for every caller.

## Success

- `list_quick_properties("ManagedSystem")` issues `GET /rest/api/uom/ManagedSystem/quick` with
  `Accept: */*` and returns the decoded JSON array as `list[str]`; `all_properties=True` issues the
  same path with `/all` appended.
...
- A 200 body that is not a JSON array of strings — invalid JSON, an object, a scalar, or an array
  holding any non-string element — raises `HMCError` naming the observed shape and returns nothing.
...
per-contract inventory — mode, test name, expected red failure, and exact focused green command — is
Task 1's `Verification` block in `docs/workflow/plans/2026-09-14-discover-quick-properties.md`, whose
one `task-test-not-applicable` contract is the `CHANGELOG.md` bullet: prose about a facade addition
that no executable consumer reads. The last criterion above is the guardrail one, discharged by the
plan's final steps.
