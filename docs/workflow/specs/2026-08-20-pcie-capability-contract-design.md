...
|---|---|---|---|---|
| Assign/unassign dedicated slot | Exact `io_slots` profile readback is admitted by ADR 0165, for HMC V10R3 M1060 on managed-system model 8375-42A only; mutation stays capability-unavailable until #882 selects that readback and enforces the same envelope, and stays unavailable unconditionally outside it; do not mutate | Exact `io_slots` profile readback is admitted by ADR 0165, for HMC V10R3 M1060 on managed-system model 8375-42A only; mutation stays capability-unavailable until #882 selects that readback and enforces the same envelope, and stays unavailable unconditionally outside it; do not mutate | Dynamic grammar is documented as `chhwres -r io -m SYSTEM -o a/r --id LPAR_ID -l DRC_INDEX`, but mutation is capability-unavailable until one family admits both it and exact readback; do not compose Power10/11 mutation evidence with Power9 read evidence; do not mutate | Any unavailable precondition or command failure is an error; do not fall back to a profile-only success |
