...
`_JOB_PATH`'s identifier segment is `[^/]+`, which admits `?` and `#`, and the
pattern is matched against the decoded path alone, which a `%2F` can turn into a
job path the raw request is not. ADR 0149 holds the evidence and the decision.

## Scope

`src/hmc_mcp/client/core.py`: tighten `_JOB_PATH`'s identifier segment to
`[^/?#]`; require it to match the raw path as well as the decoded one; give
`_reject_non_job_path` a second parameter naming the refused argument, defaulting
to `job_href`, and a remedy sentence covering both; collapse `get_job_entry`'s
branch into the two lines `delete_job` uses. New cases in
...
nothing migrates, no path becomes obsolete. The parameter default keeps
...
- **Accepted failure classes.** A `job_id` whose percent-encoding decodes to `/`,
  ...
  HMC-minted UUID or JobID carries one. A `job_href` that spells a path segment
  with an escape (`/rest/api/uom/job%73/abc`) is refused although a server would
  resolve it — accepted: the HMC emits SELF links unescaped, and refusing is the
  fail-closed half of matching both forms. A character in a `job_id` outside `/`,
  `?` and `#` still reaches `_request` (ADR 0148) — accepted: this closes a class
  boundary, not a character allowlist. Reaching a *different* job under an
  `all-targets` grant — accepted under ADR 0039.
...
- **Boundaries.** None added; one narrowed — caller-supplied `job_id` and
  `job_href` values entering job path construction in `core.py`.
...
- **Control.** `_reject_non_job_path` refuses unless the raw path *and* its
  `unquote`d form both end in `Job` or `jobs` followed by one identifier segment
  free of `?` and `#`. Refuse, never repair; the message names the argument and
  the class, never the path.
...
3. Both methods refuse a `job_href` whose path is
   `/rest/api/uom/HmcUser/root%2FJob%2Fx` — a job path only after decoding.
4. The message names `job_id` when the path came from `job_id`, `job_href` when
   it came from `job_href`.
5. Each `job_href` shape pinned by
   ...
   `wait_for_job_entry`.
6. ADR 0149 exists and `just adr-numbering` passes.
...
for criteria 1-5 and `just adr-numbering` for criterion 6.
