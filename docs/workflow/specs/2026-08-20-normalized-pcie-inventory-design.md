...
Dedicated-slot `availability` likewise remains `None`: ADR 0053 admits owner but no availability
field, and an empty owner does not prove that a slot is assignable. Callers must not infer
availability from `owner_lpar`.
