...
4. On the eligible transport-only failure, it closes the failed client, creates
   a 12443 client with the same options, and repeats the same logon request once.
...
- Failure to close the first client prevents a retry and surfaces the close
  failure; the workflow does not open a second transport while cleanup is
  uncertain.
...
- if closing the failed 443 client raises, no 12443 client or request is created
  and the close failure surfaces;
