...
Two functions in `src/hmcpctl/client/client_storage.py` own the rules. Every consumer below
calls them, and no consumer re-implements either rule:
...
- `storage_mapping_id(mapping: Mapping[str, Any]) -> str | None` takes the parsed-dict form a
  ...
  and `TargetDevice` is a dict with exactly one child. That child must be a dict whose
  `TargetName` is a non-empty string without `/`. Otherwise it returns `None`.
...
| `StorageClient.delete_storage_mapping(vios_uuid, mapping_id)` | convert each `VirtualSCSIMapping` element with `element_to_dict` and select by `storage_mapping_id`. Exactly one match is removed. Zero matches is `not found`; several is `duplicated`. Both raise `HMCError` with no POST. A mapping with no identity is ignored. An empty selector raises `ValueError`. |
...
| `detach_storage_mapping(hmc, vios, mapping_id, ...)` | match inventory by `storage_mapping_id`. Zero matches raises `ValueError` "not found"; several raises "ambiguous"; no LPAR from the href raises "does not identify its client LPAR". It then authorizes that LPAR and calls the remover. |
| `unmount_optical_media` | pass `storage_mapping_id(match)` to the remover; `None` raises `HMCError("VirtualSCSIMapping has no adapter/target identity")` |
...
3. **Accepted failure classes.**
   ...
   - A mapping change between the inventory read and the remover's GET, or between that GET
     and the POST. The remover re-selects by identity from its own GET, so a vanished or
     duplicated identity fails with no POST. ADR 0079 already puts serialization on callers.
...
4. **Covered elsewhere.**
   - Live confirmation against a lab VIOS: the #879 live window.
...
|---|---|---|
| identity derivation | focused-test | `tests/storage/test_mapping_inventory.py`: the fixture element gives `vhost0/vtscsi0`. A missing adapter name, a missing target name, two targets, and a `/` in a name each give `None`. |
...
| remover | focused-test | exact removal; not-found, duplicate, and empty selector fail with no POST; unidentifiable siblings are retained |
...
| optical unmount | focused-test | `tests/storage/test_media_operations.py`: removes by derived id; no identity raises with no removal |
