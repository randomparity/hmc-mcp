...
2. Inputs used only by the removed step are removed (operator decision, pre-release surface,
   ...
   `adapters add-vscsi` keep their inputs.
3. Authorization: removing `vios_partition_id` removes a `vios` selector from
   ...
   authorized to make. The explanatory comments beside each decorator are rewritten so they
   no longer cite the removed argument, and say the flag is kept pending a separate decision
   (follow-up candidate).
...
5. Docs: regenerate `docs/tools/`; `docs/cli.md`'s storage-model note says provision and
   ...
   a pointer to #1030; CHANGELOG `[Unreleased]` records the removal under `Removed`/`Fixed`.
...
3. Accepted failure classes: a caller still passing `vios_partition_id`/`vios_slot` gets an
   argument error (pre-release surface, no shim, CHANGELOG says so); unpaired adapters left
...
- The removed inputs are rejected by the tool signatures and CLI.
...
- `focused-test`: `tests/app/test_cli_commands.py` — CLI builds requests without the removed
  options; `tests/app/test_tool_security.py` / `tests/unit/test_tool_registry.py` — selector
  inventories without the removed selectors.
