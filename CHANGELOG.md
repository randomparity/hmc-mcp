...
- Exported model/literal changes: `HMCConfig` gained the `from_mapping(values)` classmethod
  ...
  release under the `0.x` rule. It does **not** move the frozen public signature digest: that
  digest hashes each export's `inspect.signature`, which for a pydantic model is the
  field-derived `__init__`, and a method changes no field. `api.__all__` is unchanged.
