...
async def test_set_lpar_processors_runs_inside_a_running_event_loop(mock_hmc):
    ...
    with patch(
        "hmc_mcp.operations.lpar.ownership.get_lpar_description", new=_owned_by("hmc-mcp")
    ):
...
async def test_foreign_owner_is_rejected_before_any_mutation(mock_hmc, operation):
    ...
    with patch("hmc_mcp.operations.lpar.ownership.get_lpar_description", new=_owned_by("bob")):
