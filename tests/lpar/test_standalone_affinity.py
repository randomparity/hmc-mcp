...
async def test_measurement_runs_current_prediction_and_policy_reads_after_success() -> (
    ...
    with (
        patch(
                "hmc_mcp.operations.affinity.rest.get_lpar_memopt_score",
            new=AsyncMock(return_value={"curr_lpar_score": "82"}),
        ) as current,
        patch(
                "hmc_mcp.operations.affinity.rest.plan_lpar_memopt_scores",
            new=AsyncMock(
                return_value=[{"lpar_name": "lpar-1", "predicted_lpar_score": "84"}]
            ),
        ) as predicted,
        patch(
                "hmc_mcp.operations.affinity.rest.get_minimum_affinity_policy",
...
def test_completed_activation_runs_and_returns_assessment() -> None:
    ...
    with (
        ...
        patch(
            "hmc_mcp.operations.lpar.core.power_lpar",
            new=AsyncMock(return_value=_power_result("COMPLETED")),
        ),
        patch(
            "hmc_mcp.operations.lpar.core.assess_post_activation_affinity",
            new=assessment,
        ),
    ):
