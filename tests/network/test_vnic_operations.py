...
def _common(monkeypatch: pytest.MonkeyPatch) -> None:
    module = "hmc_mcp.operations.virtualization.vnic"
    monkeypatch.setattr(
...
async def test_add_rejects_non_integer_vlan_before_preflight(
    ...
    preflight = AsyncMock()
    mutation = AsyncMock()
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic._preflight_add", preflight)
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.add_vnic_backing", mutation
    )
...
async def test_add_preserves_shell_metacharacters_as_quoted_payload_data(
    ...
    selector = VnicBackingSelector(**values, capacity_percent=Decimal(2))
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.read_vios_identity",
        AsyncMock(
            return_value={
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_sriov_adapter_rows",
        AsyncMock(
            return_value=[
                {
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_sriov_physical_port_rows",
        AsyncMock(
            return_value=[
                {
                    "adapter_id": values["adapter_id"],
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_rows", AsyncMock(side_effect=[[], []])
    )
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_backing_rows",
        AsyncMock(side_effect=[[], []]),
    )
    mutate = AsyncMock()
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic.add_vnic_backing", mutate)
...
async def test_add_rejects_exhausted_capacity(monkeypatch: pytest.MonkeyPatch) -> None:
    _common(monkeypatch)
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_sriov_configured_logical_port_rows",
        AsyncMock(
            return_value=[
                {
                    "adapter_id": "1",
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_rows", AsyncMock(return_value=[])
    )
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_backing_rows",
...
async def test_add_verified_retry_is_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    _common(monkeypatch)
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_rows",
...
async def test_add_successfully_correlates_new_slot(
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.add_vnic_backing",
...
async def test_add_ignores_unrelated_equal_selector_backing_for_target_identity(
    ...
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic.add_vnic_backing", mutation)
...
async def test_add_before_state_after_dispatch_is_known_unchanged_partial(
    ...
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic.add_vnic_backing", AsyncMock())
...
async def test_remove_success_preserves_captured_selector(
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.remove_vnic_slot",
...
async def test_remove_reconciliation_decision_table(
    ...
    if mutation_error is not None:
        mutation.side_effect = mutation_error
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic.remove_vnic_slot", mutation)
...
async def test_remove_requires_exactly_one_embedded_backing_before_mutation(
    ...
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_rows", AsyncMock(return_value=[row])
    )
    monkeypatch.setattr(
        "hmc_mcp.operations.virtualization.vnic.list_vnic_backing_rows",
        AsyncMock(return_value=[_backing()]),
    ...
    mutate = AsyncMock()
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic.remove_vnic_slot", mutate)
...
async def test_remove_successful_reads_with_changed_slot_are_contradictory(
    ...
    monkeypatch.setattr("hmc_mcp.operations.virtualization.vnic.remove_vnic_slot", AsyncMock())
