...
def test_mcp_inspect_accepts_newer_version_without_validation() -> None:
    assert hmc_snapshot_inspect(
        '{"format":"hmc-mcp.lpar-snapshot","version":2}'
    ) == {"format": "hmc-mcp.lpar-snapshot", "version": 2, "supported": False}
...
def test_cli_inspect_reads_local_file(tmp_path: Path) -> None:
    ...
    path.write_text(
        '{"format":"hmc-mcp.lpar-snapshot","version":2}', encoding="utf-8"
    )
