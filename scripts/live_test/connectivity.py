...
async def _discover_system(client: Client, state: RunState) -> None:
    ...
    if st == "PASS":
        for e in entries(data):
            ...
            if config.system_name.lower() in (resource.get("SystemName") or "").lower():
                artifacts.system_uuid = e.get("UUID")
                break
        if not artifacts.system_uuid:
            first = entries(data)
            if first:
                artifacts.system_uuid = first[0].get("UUID")
    ...
    if st == "PASS" and isinstance(data, dict) and not artifacts.system_uuid:
...
async def _discover_partitions(client: Client, state: RunState) -> None:
    ...
    if st == "PASS" and isinstance(data, dict) and not artifacts.lp3_uuid:
...
async def _discover_vios(client: Client, state: RunState) -> None:
    ...
    if st == "PASS" and not artifacts.vios_uuid:
        for e in entries(data):
            resource = get_resource(e)
