...
def record(state: RunState, subtask: int, tool: str, status: str, data: Any, note: str = "") -> None:
    entry = {
        "subtask": subtask,
        "tool": tool,
        "status": status,
...
async def subtask_0(client: Client, state: RunState) -> None:
    ...
    # 2. Composite summary
    st, data = await call(client, "hmc_lpar_summary", lpar_name_or_uuid=context["lp3_name"])
    ...
    if st == "PASS":
        ...
        for e in _entries(data):
            ...
            if pvid:
                ...
                context["lp3_baseline"]["vswitch_id"] = int(r.get("VirtualSwitchID") or r.get("virtual_switch_id") or 0)
                break
    ...
    print(f"  lp3 UUID: {context.get('lp3_uuid')}")
    print(f"  VIOS UUID: {context.get('vios_uuid')}  PartitionID: {context.get('vios_partition_id')}")
    print(f"  lp3 PVID: {context['lp3_baseline'].get('pvid')}  "
...
async def subtask_1(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST1: Connectivity & Inventory ===")
    ...
    if st == "PASS":
        for e in _entries(data):
            r = _res(e)
            if context["system_name"].lower() in (r.get("SystemName") or "").lower():
    ...
    st, data = await call(client, "hmc_systems", system_name_or_uuid=context["system_name"])
    ...
    print(f"  System UUID: {context.get('system_uuid')}")
    ...
    st, data = await call(client, "hmc_system_summary", system_name_or_uuid=context["system_name"])
...
async def subtask_2(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST2: Network Inventory ===")
    ...
    if st == "PASS":
        ...
        for candidate in range(3000, 3100):
            if candidate not in used_vlans:
                ...
                break
    print(f"  Test VLAN ID: {context.get('test_vlan_id')}  VSwitch ID: {context.get('test_vswitch_id')}")
...
async def subtask_3(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST3: Storage & SSP Inventory ===")
    ...
    if context["vios_uuid"]:
        ...
        if st == "PASS":
            for vg in _entries(data):
                ...
                for vd in (vdisks if isinstance(vdisks, list) else []):
                    vd_r = _res(vd)
                    if vd_r.get("DiskName") == context["vdisk_name"]:
        ...
        print(f"  VG UUID: {context.get('vg_uuid')}  vdisk_size_mb: {context.get('vdisk_size_mb')}")
    else:
        skip(state, 3, "hmc_list_volume_groups", "no VIOS UUID in context (ST0/ST1 failed)")
...
async def subtask_4(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST4: LPAR Properties & Profile Inventory ===")
...
async def subtask_5(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST5: Metrics & Templates ===")
    ...
    _record_expected_or_real(state, 5, "hmc_get_pcm_preferences", st, data,
    ...
    st, data = await call(client, "hmc_processed_metrics",
                          ...
                          mode="links")
    _record_expected_or_real(state, 5, "hmc_processed_metrics (links)", st, data,
                             expected_fail_substrings=["PCM", "406", "403"],
                             skip_reason="PCM not licensed on this HMC (expected)")
    ...
    st, data = await call(client, "hmc_aggregated_metrics",
                          ...
                          start_ts="2026-01-01T00:00:00.000Z",
                          mode="links")
    _record_expected_or_real(state, 5, "hmc_aggregated_metrics (links)", st, data,
                             expected_fail_substrings=["PCM", "406", "403"],
                             skip_reason="PCM not licensed on this HMC (expected)")

    st, data = await call(client, "hmc_partition_templates")
    _record_expected_or_real(state, 5, "hmc_partition_templates", st, data,
...
async def subtask_6(client: Client, state: RunState) -> None:
    print("\n=== ST6: User & Policy Inventory ===")

    st, data = await call(client, "hmc_users")
    _record_expected_or_real(state, 6, "hmc_users", st, data,
    ...
    _record_expected_or_real(state, 6, "hmc_list_password_policies", st, data,
                             expected_fail_substrings=["REST000E", "400"],
                             skip_reason="HmcPasswordPolicy REST endpoint not supported (expected)")

    st, data = await call(client, "hmc_get_ldap_config")
    _record_expected_or_real(state, 6, "hmc_get_ldap_config", st, data,
                             expected_fail_substrings=["REST000E", "400"],
...
async def subtask_7(client: Client, state: RunState) -> None:
    print("\n=== ST7: CLI Escape Hatch ===")
...
async def subtask_8(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST8: LPAR Lifecycle ===")

    if not context["system_uuid"]:
        st2, d2 = await call(client, "hmc_systems", system_name_or_uuid=context["system_name"])
    ...
    st, data = await call(client, "hmc_modify_lpar",
                          ...
                          desired_memory=768,
                          max_memory=1536)
    _record_expected_or_real(state, 8, "hmc_modify_lpar", st, data,
    ...
    st, data = await call(client, "hmc_lpar_summary", lpar_name_or_uuid=context["scratch_name"])
    record(state, 8, "hmc_lpar_summary (post-modify)", st, data)

    st, data = await call(client, "hmc_power_on_lpar",
                          lpar_name_or_uuid=context["scratch_name"],
                          wait=True)
    record(state, 8, "hmc_power_on_lpar", st, data, "boot failure expected — no OS installed")
    if st == "PASS" and isinstance(data, dict):
        context["job_uuid_sample"] = (
            data.get("job_uuid") or data.get("UUID") or context.get("job_uuid_sample")
        )
    ...
    if st == "PASS" and isinstance(data, dict) and not context.get("job_uuid_sample"):
...
async def subtask_9(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST9: Virtual Networking Mutations ===")
    ...
    st, data = await call(client, "hmc_create_virtual_network",
                          ...
                          tagged=False)
    _record_expected_or_real(state, 9, "hmc_create_virtual_network", st, data,
                             expected_fail_substrings=["406", "not acceptable"],
    ...
    if context["test_network_uuid"]:
        ...
        skip(state, 9, "hmc_add_network_adapter", "virtual network not created (REST 406)")
        skip(state, 9, "hmc_list_adapters (post-add)", "virtual network not created (REST 406)")
...
async def subtask_10(client: Client, state: RunState) -> None:
    context = state.context
    ...
    if _restore_desc and (
        ...
        skip(state, 10, "hmc_set_lpar_description (restore)",
    ...
    if lp3_env == "vioserver":
        ...
        if st_bad == "FAIL" and (
            ...
            record(state, 10, "hmc_set_lpar_msp (non-VIOS rejection — expected)",
                   "PASS", f"correctly rejected: {str(data_bad)[:200]}")
        else:
            record(state, 10, "hmc_set_lpar_msp (non-VIOS rejection)", st_bad, data_bad,
                   f"lpar_env={lp3_env!r}")
        skip(state, 10, "hmc_set_lpar_msp (toggle/verify/restore)",
    ...
    if mode and mode.lower() not in ("default", ""):
        ...
        skip(state, 10, "hmc_set_lpar_proc_compat",
...
async def subtask_11(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST11: User Administration ===")
    ...
    _record_expected_or_real(state, 11, "hmc_create_user", st, data,
                             expected_fail_substrings=_REST000E_SKIP,
                             skip_reason="HmcUser REST not supported on this HMC (expected)")
    user_created = st == "PASS"

    st, data = await call(client, "hmc_users")
    _record_expected_or_real(state, 11, "hmc_users (confirm created)", st, data,
                             expected_fail_substrings=_REST000E_SKIP,
    ...
    st, data = await call(client, "hmc_create_password_policy",
                          ...
                          min_length=10)
    _record_expected_or_real(state, 11, "hmc_create_password_policy", st, data,
                             expected_fail_substrings=_REST000E_SKIP,
                             skip_reason="HmcPasswordPolicy REST not supported (expected)")
    policy_created = st == "PASS"

    st, data = await call(client, "hmc_list_password_policies")
    _record_expected_or_real(state, 11, "hmc_list_password_policies (confirm)", st, data,
                             expected_fail_substrings=_REST000E_SKIP,
                             skip_reason="HmcPasswordPolicy REST not supported (expected)")
    ...
    if policy_created:
        ...
        skip(state, 11, "hmc_modify_password_policy", "policy not created (REST not supported)")
    ...
    if policy_created:
        ...
        skip(state, 11, "hmc_delete_password_policy", "policy not created (REST not supported)")

    st, data = await call(client, "hmc_users")
    _record_expected_or_real(state, 11, "hmc_users (confirm deleted)", st, data,
                             expected_fail_substrings=_REST000E_SKIP,
                             skip_reason="HmcUser REST not supported (expected)")

    st, data = await call(client, "hmc_list_password_policies")
    _record_expected_or_real(state, 11, "hmc_list_password_policies (confirm deleted)", st, data,
                             expected_fail_substrings=_REST000E_SKIP,
                             skip_reason="HmcPasswordPolicy REST not supported (expected)")
...
async def subtask_12(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST12: PCM Metrics & Job Monitoring ===")
    ...
    st, data = await call(client, "hmc_get_pcm_preferences",
                          category="ManagedSystem",
                          resource_name_or_uuid=context["system_name"])
    _record_expected_or_real(state, 12, "hmc_get_pcm_preferences", st, data,
                             expected_fail_substrings=["PCM", "406", "403"],
                             skip_reason="PCM not licensed on this HMC (expected)")
    current_ltm = None
    if st == "PASS" and isinstance(data, dict):
    ...
    if current_ltm is not None:
        ...
        skip(state, 12, "hmc_set_pcm_preferences", "PCM not licensed/enabled on this HMC (expected)")

    job_uuid = context.get("job_uuid_sample")
    if job_uuid:
        st, data = await call(client, "hmc_get_job", job_uuid=job_uuid)
        _record_expected_or_real(state, 12, "hmc_get_job", st, data,
        ...
        st, data = await call(client, "hmc_wait_for_job",
                              ...
                              timeout_seconds=10,
                              poll_interval=2)
        _record_expected_or_real(state, 12, "hmc_wait_for_job", st, data,
                                 expected_fail_substrings=["REST000E", "REST000B", "400"],
                                 skip_reason="Job REST type not supported on this HMC firmware")
    else:
    ...
    if not context.get("job_uuid_sample") and st == "PASS":
        for e in _entries(data):
            if isinstance(e, dict) and e.get("type") != "error":
                context["job_uuid_sample"] = e.get("UUID") or e.get("uuid")
                break
...
async def subtask_13(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST13: Provision Dry Run ===")

    # Prefer lp3's own PVID (always present on the system); fall back to test VLAN
    pvid = context["lp3_baseline"].get("pvid") or context.get("test_vlan_id")
    vios_uuid = context.get("vios_uuid")
    vios_pid = context.get("vios_partition_id") or context["lp3_baseline"].get("vios_partition_id")
...
async def subtask_14(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST14: Storage Lifecycle + Full Live Provision of ltczz386-lp3 ===")

    baseline = context["lp3_baseline"]
    vios_uuid = context.get("vios_uuid")
    vg_uuid = context.get("vg_uuid")
    vdisk_size_mb = context.get("vdisk_size_mb") or 49152
    pvid = baseline.get("pvid")
    vios_slot = baseline.get("vios_slot")
    vios_pid = context.get("vios_partition_id") or baseline.get("vios_partition_id")
    ...
    if missing:
        record(state, 14, "pre-flight check", "FAIL",
    ...
    record(state, 14, "pre-flight check", "PASS",
    ...
    if st_check == "PASS":
        ...
        skip(state, 14, "hmc_power_off_lpar", "lp3 not found — already deleted in previous run")
        skip(state, 14, "hmc_delete_lpar", "lp3 not found — already deleted in previous run")
    ...
    vg_name = context.get("vdisk_vg_name") or "VG1"
    vdisk_name = context["vdisk_name"]
    rmvlog_cmd = (
        f"viosvrcmd -m {context['system_name']} -p {context.get('vios_uuid', vios_uuid)}"
        f" -c \"rmvlog -vg {vg_name} -lv {vdisk_name}\""
    )
    st, data = await call(client, "hmc_run_command", cmd=rmvlog_cmd)
    _record_expected_or_real(state, 14, "hmc_run_command rmvlog (delete old VG1-lp3)", st, data,
    ...
    _record_expected_or_real(state, 14, "hmc_create_virtual_disk (VG1-lp3)", st, data,
                             expected_fail_substrings=["406", "not acceptable"],
...
async def subtask_15(client: Client, state: RunState) -> None:
    context = state.context
    print("\n=== ST15: Restore ltczz386-lp3 to Baseline ===")
    ...
    if _restore_desc and (
        ...
        skip(state, 15, "hmc_set_lpar_description (restore)",
             "original description contains non-ASCII/non-printable chars — cannot restore via CLI")
    else:
        st, data = await call(client, "hmc_set_lpar_description",
                              system_name_or_uuid=context["system_name"],
    ...
    if mode and mode.lower() not in ("default", ""):
        ...
        skip(state, 15, "hmc_set_lpar_proc_compat (restore)",
    ...
    try:
        ...
        for key, current in asdict(context).items():
            if current is None and saved_ctx.get(key) is not None:
                context[key] = saved_ctx[key]
            elif key == "lp3_baseline" and not current and saved_ctx.get(key):
                context[key] = saved_ctx[key]
        print(f"  ℹ  Context restored from {results_path} "
              f"(vios_uuid={context.get('vios_uuid')}, "
              f"system_uuid={context.get('system_uuid')}, "
              f"vg_uuid={context.get('vg_uuid')})")
